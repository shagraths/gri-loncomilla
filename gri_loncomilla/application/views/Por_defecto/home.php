<!DOCTYPE html>
<html>
    <head>
        <meta charset="utf-8"/>
        <script type="text/javascript" src="<?= base_url() ?>../js/jquery-1.10.2.js"></script>
        <script type="text/javascript" src="<?= base_url() ?>../js/jquery-ui-1.10.4.js"></script>
        <link type="text/css" rel="stylesheet" href="<?= base_url(); ?>../css/jquery-ui-1.10.4.css">
        <link type="text/css" rel="stylesheet"href="<?= base_url(); ?>../css/estilo.css"/>
        <script type="text/javascript" src="<?= base_url() ?>../js/funciones.js"></script>
        
<!--        time-->
        <link rel="stylesheet" type="text/css" href="jquery.timeentry.package-2.0.1/jquery.timeentry.css">
        <script type="text/javascript" src="jquery.timeentry.package-2.0.1/jquery.plugin.js"></script>
        <script type="text/javascript" src="jquery.timeentry.package-2.0.1/jquery.timeentry.js"></script>
        
        
        <script type="text/javascript">
            var base_url = '<?= base_url(); ?>';
        </script>
    <div id="cerrar_session">
        <button id="cerrar"> cerrar sesion</button>
    </div>
    <div id="logo1">
        
    </div>
    <div id="titulo"></div>

</head>

<body>
    <div id="contenido">

    </div>
</body>

<footer>
    <div id="logo2"></div>
</footer>
</html>